...
import { ConfigurationError, ExternalServiceError, NotImplementedAppError } from '@/lib/server/errors'
...
async function downloadImage(url: string, label: 'source' | 'mask'): Promise<DownloadedImage> {
  const response = await fetch(url)

  if (!response.ok) {
    const responseText = (await response.text()).trim()

    throw new ExternalServiceError(`Failed to download ${label} image for Gemini`, {
      provider: 'google',
...
async function runPromptOnlyGeminiEdit(input: GeminiEditImageInput): Promise<GeminiEditImageResult> {
  const config = getGeminiConfig()
  const sourceImage = await downloadImage(input.sourceImageUrl, 'source')
  const prompt =
    input.prompt?.trim() ||
...
export async function editImageWithGemini(input: GeminiEditImageInput): Promise<GeminiEditImageResult> {
  getGeminiConfig()

  const maskImage = await downloadImage(input.maskImageUrl, 'mask')

  if (maskImage.bytes.byteLength > 0) {
    throw new NotImplementedAppError('Gemini API-key image editing does not support explicit mask-based inpainting in this worker', {
      provider: 'google',
...
export async function editImageWithoutMaskWithGemini(
  input: Omit<GeminiEditImageInput, 'maskImageUrl'>,
): Promise<GeminiEditImageResult> {
  getGeminiConfig()

  return runPromptOnlyGeminiEdit({
    ...input,
    maskImageUrl: '',
  })
}
